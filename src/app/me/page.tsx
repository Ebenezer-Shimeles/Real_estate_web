...
export default () =>{
    ...
    const updateProfile = async () =>{
        try{
            ...
            if(file){
                ...
                const response = await axios.post('https://localhost:7113/api/users/me/picture', formData, {headers: {'Content-Type': 'multipart/form-data'}});
    ...
    return (
        <div className="h-screen w-screen flex flex-col items-center justify-center">
            <input  ref={fileRef} type='file' onChange={(e)=>setFile( Array.from(e.target.files ?? [null])[0] )}  className="hidden"/>
            <Header />
            <main className="h-3/4 w-3/5 rounded-xl bg-white flex flex-col p-5 items-center gap-3 card">
                 <div className="self-end flex  gap-3">
                  ...
                  <div className="rounded-full w-32 h-32 w-  text-3xl shadow-xl self-center bg-slate-400 flex justify-center items-center" >
                     {(session?.profileUrl) ?
                     <img className="w-full h-full rounded-full" src={`${session.profileUrl}`} />
                     :
                  ...
                  <button className="self-center underline text-blue-800" onClick={()=>fileRef.current?.click()}>Change Photo</button>

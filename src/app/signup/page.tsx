...
export default function LoginPage(){
    ...
    return (
        <div className="h-screen w-screen flex flex-row justify-center items-center bg-slate-50">
           <img src='bg_2.jpg' className="w-2/3  hidden md:block h-screen  bg-cover"/>
            
            <form onSubmit={handleSend} className="flex flex-col sm:w-full md:w-1/3   h-3/5 rounded-2xl justify-center gap-2 items-center">
                <h2 className="text-3xl  underline mb-8 font-bold">Register For An Account</h2>
                <input type ='text' placeholder="First Name" className="rounded-lg p-1 bg-white border-2 border-black w-3/4" 
                  onChange={e => setFirstName(e.target.value)} required
                />

                <input type ='text' placeholder="Last Name" className="rounded-lg p-1 bg-white border-2 border-black w-3/4" 
                  onChange={e => setLastName(e.target.value)} required
                />

                <input type ='email' placeholder="Email Address" className="rounded-lg p-1 bg-white border-2 border-black w-3/4" 
                  onChange={e => setEmail(e.target.value)} required
                />
                <input type ='number' placeholder="Phone Number" className="rounded-lg p-1 bg-white border-2 border-black w-3/4" 
                  onChange={e => setPhoneNumber(e.target.value)} required
                />


                <div className="w-full flex justify-center items-center ml-7 gap-4 ">
                    <input  type = {!isPasswordVsisible?'text':'password' }  placeholder="Password"   
                   ...
                   <button onClick={handleEye} className="w-1/7 flex flex-col justify-cetner h-2/3">
                     {isPasswordVsisible ? <FaEyeSlash/> :  <FaEye />}   
                    </button>
                </div>
                {error && <label className="text-red-700 text-sm">{error}</label>}
                <button className="bg-green-500 text-white mb-20 pl-20 pr-20 pt-2 pb-2 rounded-xl font-bold hover:bg-green-600">Submit</button>
                <p className="mb-10 text-sm"> Already have an account? <Link className="underline text-blue-500" href={'login'}>Login</Link></p>
                <button
                   onClick={(e)=>{
                    e.preventDefault();
                    oauthSignIn()
